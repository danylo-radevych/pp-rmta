  ...
  !
    ...
    PUBLIC :: &
      ...
      nneighbors, inn_i, nn_dist, nr_dist
    !
    ...
    REAL(DP), ALLOCATABLE :: nr_dist(:, :)
    !! distances to the nearest replicas of each atom
  !
  CONTAINS
  !
    !
    ...
    !---------------------------------------------------------------------------
      ...
      !
      DEALLOCATE(nr_dist, STAT = ierr)
      IF (ierr /= 0) CALL errore(routine_name, 'Error deallocating nr_dist', 1)
      !
    !---------------------------------------------------------------------------
