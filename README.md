...
| `LOGICAL`    | `lrmt_only`  | `.false.` | if `.true.`, stop execution as soon as possible after displaying the MT radii |
| `LOGICAL`    | `ltetra`     | `.true.`  | if `.true.`, use tetrahedron method for partial DOS integration; if `.false.`, use delta-function smearing set by `ngauss` and `degauss` |
...
| `INTEGER`    | `nrmt`       | `1`       | number of points on the logarithmic radial grid, at or below the MT radius, at which the partial DOS and electronic factors are evaluated and printed to the corresponding `etall1rf.dat` and `dos_*.dat` files |
