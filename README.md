...
| `CHARACTER`  | `rmt_method` | `'touching'`| see `lrmt`: if `'default'`, set default MT radii from the table (results in very small MT radii); if `'pseudo'`, set default MT radii as pseudopotential cutoff radii; if `'neighbor'`, divide nearest-neighbor distances into ratios of the default MT radii (still leaves a lot of empty space); if `'touching'`, starting from `'neighbor'`, enforce touching spheres (recommended); if `'psedoneighbor'`, divide nearest-neighbor distances into ratios of the pseudopotential cutoff radii; if `'pseudotouching'`, starting from `'pseudoneighbor'`, enforce touching spheres.|
