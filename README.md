...
| :---         | :---         | :---:     | :---                               |
| `CHARACTER`  | `formulation`| `'default'` | if `'nodeless'`, calculate partial DOS based on continuous wavefunctions; if `'inflectionless'`, calculate partial DOS based on continuous derivatives of the wavefunctions; if `'default'`, automatically activate `'inflectionless'` when `'nodeless'` is close to radial nodes (recommended) |
